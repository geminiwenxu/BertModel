...
class STC(TextScore):
    ...
    def score(self, text: Text):
        ...
        df = pd.DataFrame(list(l) for l in zip(np.arange(1,
                                                         len(text.token_frequencies[:h_floor]) + 1),
                                               text.token_frequencies))
...
class Scorer():
    ...
    def __init__(self, scorers, out_dir='', out_file=None, tfidf=False, tfidf_max=10,
        ...
        self.mode = mode
        self.cpus = cpus
        self.out_dir = '../../../Downloads/'  # os.path.join('data/scores/cumulative', out_dir)
...
if __name__ == "__main__":
    ...
    scores, names, text_hash = sc.run("de", "dummy", "hallo, hallo, hallo")
    print(scores)
    print(len(scores))
